...
function preloadBossIdle(bossId){
  if(!_ready || !_game || !bossId) return Promise.resolve(false);
  const scene = _game.scene.getScene('BossScene');
  if(!scene || typeof scene.preloadIdle !== 'function') return Promise.resolve(false);
  return scene.preloadIdle(bossId, 'idle').then(() => true);
}
...
function preloadBossAttacked(bossId){
  ...
  const scene = _game.scene.getScene('BossScene');
  if(!scene || typeof scene.preloadIdle !== 'function') return Promise.resolve(false);
  return scene.preloadIdle(bossId, 'attacked').then(() => true);
}
...
function preloadBossSlash(){
  ...
  const scene = _game.scene.getScene('BossScene');
  if(!scene || typeof scene.preloadSlash !== 'function') return Promise.resolve(false);
  return scene.preloadSlash().then(() => true);
}
...
function preloadBossBattleUIAssets(){
  if(!_ready || !_game) return Promise.resolve(false);
  const scene = _game.scene.getScene('BossScene');
  if(!scene || typeof scene.preloadBattleUIAssets !== 'function') return Promise.resolve(false);
  return scene.preloadBattleUIAssets().then(() => true);
}

/**
 * Précharge le fond d'arène du boss donné (voir BOSS_ARENA_BG dans BossScene.js — un fond
 * par boss désormais, plus un seul fond partagé). Renvoie une Promise, comme les autres
 * fonctions de préchargement.
 * @param {string} bossId
 * @returns {Promise<boolean>}
 */
function preloadBossArenaBg(bossId){
  if(!_ready || !_game || !bossId) return Promise.resolve(false);
  const scene = _game.scene.getScene('BossScene');
  if(!scene || typeof scene.preloadArenaBg !== 'function') return Promise.resolve(false);
  return scene.preloadArenaBg(bossId).then(() => true);
}
