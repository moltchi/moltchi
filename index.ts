      ...
      case "auto_equip":
        {
          ...
          // Score de puissance équitable entre les 2 structures d'objet existantes :
          // - Tour/Sanctuaire : stat unique ou double (value [+ value2])
          // - Noyau : bonus uniforme sur les 4 stats (allStat) — PAS de champ value du tout.
          // L'ancien tri (b.value - a.value) donnait NaN pour les objets du Noyau (value
          // undefined), les reléguant de fait en fin de liste peu importe leur vraie force —
          // un allStat de 780 (soit 3120 au total sur les 4 stats) doit pourtant largement
          // dominer un objet à 2000+650 (2650 au total). Voir la conversation.
          // ⚠️ DOIT rester cohérent avec itemPowerScore côté app.js si dupliqué un jour.
          const itemPowerScore = (i) => i.allStat ? i.allStat * 4 : (i.value || 0) + (i.value2 || 0);
          const seenDefIds = new Set();
          const nonUnique = creature.inventory.filter((i)=>i.rarity !== "unique").sort((a, b)=>itemPowerScore(b) - itemPowerScore(a)).filter((i)=>{
